...
func ListModerationReports(ctx context.Context, db *sql.DB, p types.ListModerationReportsParams) ([]types.ModerationReport, error) {
	...
	rows, err := db.QueryContext(ctx, `
		SELECT
			...
			COALESCE(NULLIF(TRIM(CONCAT_WS(' ', reporter.first_name, reporter.last_name)), ''), reporter.email),
			mr.content_member_id,
			COALESCE(NULLIF(TRIM(CONCAT_WS(' ', content_member.first_name, content_member.last_name)), ''), content_member.email),
			mr.content_summary,
			mr.reporter_details,
			mr.status,
