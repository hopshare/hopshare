...
func Load() Config {
	return Config{
		...
		ExpireNotificationAge:      getenvDuration("HOPSHARE_WORKER_EXPIRE_NOTIFICATION_AGE", 5*24*time.Hour),
		ExpireNotificationInterval: getenvDuration("HOPSHARE_WORKER_EXPIRE_NOTIFICATION_INTERVAL", 24*time.Hour),
	}
}
...
func parseDuration(raw string) (time.Duration, error) {
	...
	if strings.Count(raw, "d") != 1 {
		return time.ParseDuration(raw)
	}

	daySep := strings.Index(raw, "d")
	dayPart := raw[:daySep]
	rest := raw[daySep+1:]
