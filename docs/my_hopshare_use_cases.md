...
* At the top of the page should be a celebration of Hop activity in the current Organization- showing a few recent Hops that were completed along with basic metrics (how long it's been around, how many members, how many Hops pending / completed).

## Seeing their current stats

* Current balance of credits
* Number of Hops they've requested, along with Date of last one
* Number of Hops they've given, along with Date of last one

## Requesting Hops for help

* Should be highly visible how to do this, and we want to encourage Members to do this as often as they can.
* There should be a list of the Member's Hops- this is their 'bank statement' showing pending, canceled, expired and completed Hops. List should be filtered by date range, text, Member.
* Hops should be lifecycled here- Pending can be Canceled or Completed. Completed hops can be selected so comments / pictures can be attached.

## Finding Hops they can help with

* Member should see a list of all unfilled Hops for the current Organization.
* The list should be searchable based on text or Member. You can also have a persistent check box called "Match Me" which will automatically match Hops for the Member when they view the page.


# Having a consistent List component

Because so much of the interaction with hopShare involves going through lists of Hops (either to fulfil them, or to review what you've done), we need a good, reusable list component. While the implementation might be done by different parts of the code, the visual appearance should be the same across all lists to keep things consistent for users.
